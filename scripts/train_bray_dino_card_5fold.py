...
from lightning_models.lightning_card_classifier import LightningCARDClassifier
...
wandb_group = f"kfold_card_dino_top{num_classes}"
wandb_project = "bray_dino_card_5fold"
...
if __name__ == "__main__":
    for fold in range(5):
        ...
        checkpoint_callback = ModelCheckpoint(
            ...
            monitor="val/roc_auc",
            mode="max",
            dirpath=f"./checkpoints/bray_dino/card_5fold/fold_{fold}",
            filename="card-{epoch:02d}-{val/roc_auc:.4f}",
        )
    ...
    for k in range(num_classes):
        ...
        if m.sum() > 0 and len(np.unique(all_y_true[m, k])) > 1:
            pooled_per_class_auc.append(roc_auc_score(all_y_true[m, k], all_y_pred[m, k]))
        else:
    ...
    ax_dist.set_title("Score distribution on true-positive samples — pooled 5 folds (CARD)")
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()

    fig_auc, ax_auc = plt.subplots(figsize=(max(6, num_classes * 1.4), 5))
    sns.boxplot(data=[auc_matrix[:, k] for k in range(num_classes)], ax=ax_auc, palette="Set2")
    ...
    ax_auc.set_title(f"Per-class ROC-AUC across 5 folds — top {num_classes} MoAs (CARD)")
    plt.tight_layout()
