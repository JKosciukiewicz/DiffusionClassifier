...
class MLPClassifierBBBC(nn.Module):
    """
    MLP classifier for BBBC (Broad Bioimage Benchmark Collection) datasets.

    Features configurable activation functions and optional residual connections
    with a specialized architecture for bioimage feature classification.
    ...
    def __init__(
        ...
        # Input projection: embedding_dim -> 768
        self.input_proj = nn.Linear(embedding_dim, 768)
        self.input_norm = nn.LayerNorm(768)

        # Progressive reduction: 768 -> 512 -> 256 -> 128 -> num_classes
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        """
        Forward pass through BBBC classifier.

        Args:
            features: Input tensor of shape (batch_size, embedding_dim)

        Returns:
            torch.Tensor: Sigmoid predictions of shape (batch_size, num_classes)
        """
        # Input projection
