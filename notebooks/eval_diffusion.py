...
config_path = "configs/diffusion_bray.yaml"
result_dir_path = "./results_diffusion_cp"
os.makedirs(result_dir_path, exist_ok=True)
checkpoint_path_full = "/Users/jkosciukiewicz/Developer/UJ/Projects/HCS-DFC/checkpoints_fixed/bray_cp/diffusion/md/epoch=epoch=9-val_roc_auc=validation_roc_auc=0.63.ckpt"
...
for alpha in np.arange(0.05, 0.55, 0.05):  # Fixed range to include 1.0
    ...
    if not os.path.isfile(results_path):
        ...
        #         ]
        #
        #         print(f"Running: alpha={alpha}, size={size}, residual={residual_bool}")
        #         print(f"Results will be saved to: {results_path}")
        #
        #         try:
        #             result = subprocess.run(cmd, capture_output=True, text=True, check=True)
