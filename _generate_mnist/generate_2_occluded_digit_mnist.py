...
class SampleType(str, Enum):
    ...
    FULL = "full"  # both digits rendered
    A_ONLY = "a_only"  # only digit-A rendered; digit-B label present but invisible
    B_ONLY = "b_only"  # only digit-B rendered; digit-A label present but invisible
    NEITHER = "neither"  # neither digit rendered (stress-test extreme)
...
def _assign_sample_types(
    ...
    occlusion_frac : fraction of samples where *exactly one* digit is absent.
                     Within this pool the split between A_ONLY and B_ONLY is 50/50.
    neither_frac   : fraction of samples where *both* digits are absent (extreme case).
                     Must satisfy  occlusion_frac + neither_frac <= 1.
    """
    ...
    # split occluded evenly between A_ONLY and B_ONLY
...
def generate_dataset(
    ...
    """
    Generate a dataset of canvas images, each containing (up to) two MNIST digits.

    Occlusion control
    -----------------
    occlusion_frac : fraction of samples where exactly one digit is *not rendered*
                     but its class label is still included in the two-hot vector.
                     Splits 50/50 between A_ONLY (left digit absent) and
                     B_ONLY (right digit absent).
    neither_frac   : fraction of samples where *both* digits are absent — the canvas
                     is blank while the label still encodes both classes.

    Returns
    -------
    images   : (num_samples, canvas_size, canvas_size) uint8
    labels   : (num_samples, 10)                       float32 two-hot
    metadata : dict with keys
                 "label_a"      – int8  (N,)  digit class placed in the left slot
                 "label_b"      – int8  (N,)  digit class placed in the right slot
                 "visible_a"    – bool  (N,)  True iff digit-A was rendered
                 "visible_b"    – bool  (N,)  True iff digit-B was rendered
                 "sample_type"  – str   (N,)  one of SampleType values
    """
    ...
    for i in tqdm(range(num_samples), desc="Generating samples"):
        ...
        # composite in back-to-front order (B first, then A on top)
        if render_b:
...
def save_dataset_with_splits(
    ...
    CSV columns
    -----------
    image_path, digit_0 … digit_9,
    label_a, label_b, visible_a, visible_b, sample_type
    """
...
if __name__ == "__main__":
    ...
    images, labels, metadata = generate_dataset(
        mnist_dataset=mnist,
        num_samples=10000,
        canvas_size=128,
        max_jitter=20,
        scale_range=(0.9, 1.1),
        # --- occlusion control ---
        # 30% of samples have exactly one digit absent (label still has both)
        # split evenly: 15% a_only, 15% b_only
        occlusion_frac=0.30,
    ...
    save_dataset_with_splits(
        ...
        labels=labels,
        metadata=metadata,
        dataset_dir="_data/dual_mnist_occluded/raw",
        train_frac=0.70,
        val_frac=0.15,
        seed=42,
    )
    ...
    print(
        f"Example meta : label_a={metadata['label_a'][0]}  "
        f"label_b={metadata['label_b'][0]}  "
        f"visible_a={metadata['visible_a'][0]}  "
        f"visible_b={metadata['visible_b'][0]}  "
        f"type={metadata['sample_type'][0]}"
    )
